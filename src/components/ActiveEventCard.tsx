...
export default function ActiveEventCard({ event }: { event: EventDoc }) {
  ...
  if (status === 'upcoming') {
    ...
    return (
      <a
        ...
        className="block mb-6 rounded-xl border border-gray-200/50 bg-white/60 backdrop-blur-sm shadow-sm relative overflow-hidden h-24 sm:h-32 hover:shadow-md transition-all duration-300"
      >
  ...
  return (
    <a
      href={href}
      target={event.externalUrl ? '_blank' : undefined}
      className="block mb-6 rounded-xl border-2 border-red-400/60 bg-gradient-to-r from-red-50/90 to-pink-50/90 shadow-xl backdrop-blur-sm relative overflow-hidden h-32 sm:h-40 hover:shadow-2xl transition-all duration-300"
    >
      {/* 目立つ背景エフェクト */}
